<template>
    <div class="tab-content" id="top-tabContent">
        <div class="tab-pane fade" :class="props.data == 3 ? 'active show' : ''" id="prty-1">
            <div class="row list-property column-space no-slider-property">
                <div class="col-xl-4 col-lg-6" v-for="(item, index) in alldata.slice(12, 15)" :key="index">
...
import List from '~/static/data/types/classiclist'
interface Classic {
    icon: string;
    name: string;
}
...
let { data: icon } = await useFetch(useurl + '/data/looking-icon.json')
