...
<div class="col-xl-7">
    <div class="property-2 row column-sm zoom-gallery property-label property-grid" :class="lview">
        <div class="wow fadeInup" :class="classes" v-for="(item , index) in getdata()" :key="index">
                ...
                <div class="property-image btn-color">
                    <swiper :loop="true" navigation :modules="[Navigation , Pagination]" :pagination="{clickable:true}" class="property-slider">
                        <swiper-slide v-for="(img , index) in item.img" :key="index">
                            <a href="javascript:void(0)" :style="'background-image:url('+ img +')'" class="bg-size background">
                    ...
                    <div class="overlay-property-box">
                        <nuxt-link to="javascript:void(0)" class="effect-round" data-bs-toggle="tooltip" data-bs-placement="left" title="compare" @click="compare(item)">
                            <Icon name="material-symbols:shuffle"/>
                        </nuxt-link>
                        <nuxt-link to="javascript:void(0)" class="effect-round like added" data-bs-toggle="tooltip" data-bs-placement="left" title="wishlist" @click="wishlist(item)">
                            <Icon :name="fill?.find((ele) => ele.id == item.id)?'ph:heart-straight-fill':'ph:heart-straight-thin'" class="text-black"/>
                        </nuxt-link>
                    </div>
                </div>
    ...
    <nav class="theme-pagination" v-if="props.data.length > parpagedata">
        <ul class="pagination">
            ...
            <li class="page-item" :class="currentpage == page?'active':''" v-for="(page , index) in  totalpage" :key="index">
...
import DataType from '~/static/data/types/letestforrent'
interface MyProps{
    data: DataType[];
    classes: string;
    lview: string;
...
let compareproduct= ref<DataType[]>([])
let favproduct = ref<DataType[]>([])
function compare(value:DataType){
    compareproduct.value.push(value)
    store.compare(compareproduct.value)
}
function wishlist(value:DataType){
