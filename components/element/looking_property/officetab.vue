<template>
    <div id="tabsContent" class="tab-content">
        <div id="morden" class="tab-pane fade " :class="props.content == 'office' ? 'show active' : ''">
            <div class="property-2 row column-space zoom-gallery">
                <div class="col-xl-4 col-md-6" v-for="(item, index) in props.data?.slice(14, 17)" :key="index">
                    <div class="property-box orange btn-color">
                        <swiper navigation :modules="[Navigation, Pagination]" :pagination="{ clickable: true }"
                            class="property-image">
                            <swiper-slide class="property-slider" v-for="(img, index) in item.img" :key="index">
...
onMounted(() => {
    ...
    if (storedData) {
        try {
            const parsedData: DataType[] = JSON.parse(storedData);
