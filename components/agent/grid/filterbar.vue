        ...
        <div class="filter-cards">
            ...
            <div class="advance-card feature-card">
                ...
                <swiper :loop="true" class="feature-slider">
                    <swiper-slide v-for="(item, index) in single" :key="index" class="bg-size background" :style="'background-image:url('+item.src+')'">
...
declare function useFetch(url: string): Promise<SingleResponse>;
const { data: addedData } = await useFetch(useurl + '/data/left-sidebar-filter.json');
const { data: singleData } = await useFetch(useurl + '/data/propertysingleiamge.json');
const single: single[] = singleData.value.feature;
const added: added[] = addedData.value.added;
</script>
